  ...
  async function saveScore(name, score){
    ...
    try{
      ...
      const existingPlayer = scores.find(entry => entry.name === name);

      let result
      if(existingPlayer){
        console.log(`${name} exists. Updating score...`);


        // Use PUT with filter for existing row ID
        result = await fetch(`https://sheetdb.io/api/v1/6e5u5wjlw0ozg/name/${encodeURIComponent(name)}`, {
          method: 'PUT',
          headers: {
        ...
        console.log(`${name} does not exist. Creating new row...`);
        //POST new player
        result = await fetch(`https://sheetdb.io/api/v1/6e5u5wjlw0ozg`, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
